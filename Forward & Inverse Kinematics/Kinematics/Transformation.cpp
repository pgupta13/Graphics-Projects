...
Quaternion Quaternion::Slerp(float t, const Quaternion& q0, const Quaternion& q1)
{
	...
	/* Quaternion x = operator -(q0,q1);
	x= operator *(t,x);
	q= operator +(q1,x); */
	float a = sin(angle*(1 - t)) / sin(angle);
	float b = sin(t*angle) / sin(angle);
	return (operator *(a, q0) + operator *(b, q1));
}
...
Quaternion Quaternion::Intermediate (const Quaternion& q0, const Quaternion& q1, const Quaternion& q2)
{
	...
	Quaternion a = Log(operator *(inv, q2));
	Quaternion b = Log(operator *(inv, q0));
	Quaternion inpExp = (a + b) / (-4);

	Quaternion q = operator *(q1, Exp(inpExp));
	return q;
}
...
Quaternion Quaternion::Squad(float t, const Quaternion& q0, const Quaternion& a, const Quaternion& b, const Quaternion& q1)
{
	...
	Quaternion x = Slerp(t, q0, q1);
	Quaternion y = Slerp(t, a, b);
	Quaternion q = Slerp(2 * t*(1 - t), x, y);
	return q;
}
